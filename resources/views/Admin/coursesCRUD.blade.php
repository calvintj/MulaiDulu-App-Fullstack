...
@section('content')
    <div class="container p-5">
                    ...
                    <tbody>
                        @foreach ($courses as $c)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('Admin.coursesCRUD.show', $c->id) }}" class="btn btn-info btn-sm">View</a>
                                    <a href="{{ route('Admin.coursesCRUD.edit', $c->id) }}" class="btn btn-warning btn-sm">Edit</a>
                                    <form action="{{ route('Admin.coursesCRUD.destroy', $c->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
        ...
        @if(isset($course) || !empty($courses))
            <form action="{{ isset($course) ? route('Admin.coursesCRUD.update', $course->id) : route('Admin.coursesCRUD.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
